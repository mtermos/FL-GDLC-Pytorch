...
from src.graph.centralities.centrality_utils import getSparseA, getGroupIndicator, getDegMat
...
def getInternalExternalDegrees(g, part, loops=True):
    ...
    A = getSparseA(g)
    group_ind = getGroupIndicator(g, cols, rows)
    node_deg_by_group = A * group_ind
    internal_degrees = node_deg_by_group[rows, cols]
    if loops:
        internal_degrees += A.diagonal()
    external_degrees = node_deg_by_group.sum(
        1).transpose() - node_deg_by_group[rows, cols]
...
def getGroupFraction(g, part):
    internal_degrees, external_degrees = getInternalExternalDegrees(
        g, part, False)
    group_ind = getGroupIndicator(g, part.membership)
...
def masuda(g, part):
    ...
    for edge in group_G.es:
        if edge.is_loop():
            edge['weight'] = 0
    try:
        group_eigs, group_lambda = group_G.eigenvector_centrality(
            return_eigenvalue=True, weights='weight')
    except:
        return [0] * g.vcount()
    if group_lambda == 0:
        return [0] * g.vcount()
    ...
    node_deg_by_group = node_deg_by_group - \
        node_deg_by_group.multiply(group_ind)
...
def community_degree(g, part):
    ...
    group_degs = (deg_mat + diags(A.diagonal())
                  * group_indicator_mat).sum(0)
